...
@override_settings(CACHES={
    'default': django_cache_url.parse('locmem://hub_test')}, DEBUG=True)
# @override_settings(CACHES = {
#    'default': django_cache_url.parse('pymemcached://127.0.0.1:11211')})
class GeneralCachingTestCase(WithUserSuperuserTestCase):
    ...
    def setUp(self):
        ...
        self.url_ct = reverse(
            'browse:browse', kwargs={'ct': 'academicprogram'})
    ...
    def run_topic_toolkit_test(self):
        ...
        self.assertNotContains(
            response, '<strong>Second Academic Program', status_code=200)
        ...
        self.assertTrue(
            uncached_request_query_count > cached_request_query_count)
    ...
    def test_topic_toolkit_tab(self):
        ...
        self.assertEqual(
            self.ct1.permission, ContentType.PERMISSION_CHOICES.member)
    ...
    def test_topic_stars_tab(self):
        ...
        response = self.client.get(self.url_topic)
        self.assertContains(
            response, "This is the STARS tab!!", status_code=200)
        ...
        response = self.client.get(self.url_topic)
        self.assertContains(
            response, "This is [not] the STARS tab!!", status_code=200)
    ...
    def run_detail_view_test(self):
        ...
        self.assertContains(
            response, '<h2>First Academic Program', status_code=200)
        ...
        self.assertContains(
            response, '<h2>First Academic Program', status_code=200)
        self.assertNotContains(
            response, '<h2>Renamed Academic Program', status_code=200)
        ...
        self.assertContains(
            response, '<h2>Second Academic Program', status_code=200)
